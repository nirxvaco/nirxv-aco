...
export default function DropsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: userProfile } = await supabase.from('user_profiles').select('pkc_opt_out').eq('id', user.id).single()
  ...
  async function toggleOptOut() {
    ...
    setPkcOptOut(newVal)
    notifyDiscord(newVal ? 'pkc_opt_out' : 'pkc_opt_in', { status: newVal ? 'Opted OUT of PKC' : 'Opted back INTO PKC' }, profile?.username)
  ...
  function openSubmit(drop) {
    ...
    const validProfileIds = existingProfileIds.filter(id => myProfiles.some(p => p.id === id))
  ...
  // Drop has items = items are required before submit
  const dropHasItems = (submitModal?.items || []).length > 0
  const itemsRequired = dropHasItems
  const canSubmit = selectedProfiles.length > 0 && (!itemsRequired || selectedItems.length > 0)
  ...
  async function handleClearProfiles() {
    setClearing(true)
    await supabase.from('drop_submissions').delete().eq('drop_id', submitModal.id).eq('user_id', user.id)
    notifyDiscord('drop_opt_out', { drop_name: submitModal.name }, profile?.username)
    await load()
    setSelectedProfiles([]); setSelectedItems([]); setSubmissionNote(''); setSubmitted(false); setClearConfirm(false); setClearing(false)
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {dropItems.length > 0 && (
                <span className="vault-badge border text-[10px] text-vault-accent bg-vault-accent/10 border-vault-accent/20 shrink-0">
                  {dropItems.length} item{dropItems.length !== 1 ? 's' : ''}
                </span>
              )}
            </div>
            {drop.drop_date && <p className="text-vault-muted text-xs font-mono"><Clock className="w-3 h-3 inline mr-1" />{format(new Date(drop.drop_date), 'dd MMM yyyy')}</p>}
          </div>

          <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto shrink-0">
            {drop.guide_hor && <a href={drop.guide_hor} target="_blank" rel="noopener noreferrer" className="vault-btn-ghost text-xs px-2.5 py-1.5 border-vault-gold/30 text-vault-gold hover:bg-vault-gold/10"><BookOpen className="w-3.5 h-3.5" /> HoR</a>}
            {drop.guide_lunar && <a href={drop.guide_lunar} target="_blank" rel="noopener noreferrer" className="vault-btn-ghost text-xs px-2.5 py-1.5 border-vault-purple/30 text-vault-purple hover:bg-vault-purple/10"><BookOpen className="w-3.5 h-3.5" /> Lunar</a>}
            {drop.guide_rv && <a href={drop.guide_rv} target="_blank" rel="noopener noreferrer" className="vault-btn-ghost text-xs px-2.5 py-1.5 border-vault-accent/30 text-vault-accent hover:bg-vault-accent/10"><BookOpen className="w-3.5 h-3.5" /> RV</a>}
            <button onClick={() => setExpandedDrop(isExpanded ? null : drop.id)} className="vault-btn-ghost text-xs px-2.5 py-1.5">
          ...
          <div className="mt-4 pt-4 border-t border-vault-border animate-fade-in space-y-3">
            {drop.notes && (
              <div className="bg-vault-gold/5 border border-vault-gold/20 rounded-xl p-3">
                <p className="text-[10px] font-mono text-vault-gold uppercase tracking-widest mb-1.5"><AlertCircle className="w-3 h-3 inline mr-1" />Admin Notes</p>
                <p className="text-vault-text text-sm whitespace-pre-wrap">{drop.notes}</p>
              </div>
            )}
            {dropItems.length > 0 && (
              <div>
                <p className="text-[10px] font-mono text-vault-muted uppercase tracking-widest mb-2">Available Items</p>
            ...
            {isSubmitted && (
              <div className="bg-vault-green/5 border border-vault-green/20 rounded-xl p-3">
                ...
                <p className="text-sm font-body text-vault-text"><span className="text-vault-text-dim">Profiles: </span>{submittedProfiles.join(', ') || '—'}</p>
                {submittedItems.length > 0 && <p className="text-sm font-body text-vault-text mt-0.5"><span className="text-vault-text-dim">Items: </span>{submittedItems.join(', ')}</p>}
                <p className="text-[10px] font-mono text-vault-muted mt-1.5">Submitted {format(new Date(submission.submitted_at), 'dd MMM yyyy HH:mm')}</p>
              </div>
            )}
          </div>
        )}
    ...
    return (
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-mono px-2 py-0.5 rounded-full border ml-1" style={{ color: colour, borderColor: `${colour}30`, background: `${colour}10` }}>{sectionDrops.length}</span>
        </div>
        <div className="space-y-3">{sectionDrops.map(drop => <DropCard key={drop.id} drop={drop} />)}</div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in" style={{ fontFamily: "'Outfit', sans-serif" }}>
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        ...
        <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${pkcOptOut ? 'bg-vault-red/5 border-vault-red/30' : 'bg-vault-bg border-vault-border'}`}>
          <div className="min-w-0">
            ...
            <p className="text-xs font-mono text-vault-muted">{pkcOptOut ? '❌ You are opted out of PKC' : '✅ You are being run on PKC'}</p>
          </div>
          <button onClick={toggleOptOut} disabled={savingOptOut}
      ...
      {loading ? (
        ...
        <div className="vault-card text-center py-16"><Package className="w-10 h-10 text-vault-muted mx-auto mb-3" /><p className="text-vault-text font-display">No drops right now</p><p className="text-vault-text-dim text-sm mt-1">Check back soon</p></div>
      ) : (
        <>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-3 sm:p-4"
          ...
          <div className="w-full max-w-lg flex flex-col animate-fade-in rounded-2xl overflow-hidden border border-vault-border"
            ...
            <div className="relative shrink-0 px-4 sm:px-6 pt-5 pb-4"
              ...
              <button onClick={() => setSubmitModal(null)} className="absolute top-3 right-3 w-8 h-8 rounded-lg bg-vault-bg/60 flex items-center justify-center text-vault-muted hover:text-vault-text transition-all">
                <X className="w-4 h-4" />
              </button>
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                <span className={`vault-badge border text-[10px] ${siteColour(submitModal.site)}`}>{submitModal.site}</span>
                {submitModal.drop_date && <span className="text-[10px] font-mono text-vault-muted flex items-center gap-1"><Clock className="w-3 h-3" />{format(new Date(submitModal.drop_date), 'dd MMM yyyy')}</span>}
              </div>
              ...
              {(submitModal.guide_hor || submitModal.guide_lunar || submitModal.guide_rv) && (
                <div className="flex gap-2 mt-3 flex-wrap">
                  {submitModal.guide_hor && <a href={submitModal.guide_hor} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold" style={{ background: 'rgba(255,230,0,0.1)', color: '#ffe600', border: '1px solid rgba(255,230,0,0.25)' }}><BookOpen className="w-3 h-3" /> HoR <ExternalLink className="w-3 h-3 opacity-60" /></a>}
                  {submitModal.guide_lunar && <a href={submitModal.guide_lunar} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold" style={{ background: 'rgba(180,79,255,0.1)', color: '#b44fff', border: '1px solid rgba(180,79,255,0.25)' }}><BookOpen className="w-3 h-3" /> LunarFBA <ExternalLink className="w-3 h-3 opacity-60" /></a>}
                  {submitModal.guide_rv && <a href={submitModal.guide_rv} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold" style={{ background: 'rgba(0,200,255,0.08)', color: '#00c8ff', border: '1px solid rgba(0,200,255,0.2)' }}><BookOpen className="w-3 h-3" /> ResellVault <ExternalLink className="w-3 h-3 opacity-60" /></a>}
                </div>
              )}
            </div>
            ...
            <div className="overflow-y-auto overflow-x-hidden flex-1 px-4 sm:px-6 py-4 space-y-4">
                ...
                <div className="rounded-xl p-4 border border-vault-red/40 bg-vault-red/5 animate-fade-in">
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-mono text-vault-muted mt-0.5">Your profiles will be removed. You can sign up again any time before it closes.</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => setClearConfirm(false)} className="flex-1 py-2 rounded-lg text-sm font-medium text-vault-text-dim hover:text-vault-text hover:bg-vault-border transition-all border border-vault-border">Keep my sign-up</button>
                    <button onClick={handleClearProfiles} disabled={clearing}
                      className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-semibold transition-all"
                      style={{ background: 'rgba(255,75,75,0.15)', color: '#ff4b4b', border: '1px solid rgba(255,75,75,0.3)' }}>
                      {clearing ? <><div className="w-3.5 h-3.5 border-2 border-vault-red border-t-transparent rounded-full animate-spin" />Removing...</> : <><Trash2 className="w-3.5 h-3.5" />Yes, remove me</>}
                    </button>
                  </div>
                </div>
              )}
              ...
              {/* Admin notes */}
              {submitModal.notes && (
                <div className="rounded-xl p-3 flex gap-3" style={{ background: 'rgba(255,230,0,0.05)', border: '1px solid rgba(255,230,0,0.2)' }}>
              ...
              {/* ── Item selection — required if drop has items ── */}
              {items.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-vault-red text-[11px] font-mono">* required</span>
                    {selectedItems.length > 0 && (
                      <span className="ml-auto text-[11px] font-mono text-vault-accent">{selectedItems.length} selected</span>
                    )}
                  </div>
                  <p className="text-[10px] font-mono text-vault-muted mb-2">Select all items you want — you can pick multiple for multicart</p>
                  <div className="flex gap-2 flex-wrap">
                    {items.map((item, i) => {
                      ...
                      return (
                        <button key={i} type="button" onClick={() => toggleItem(key)}
                          className="flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-mono font-semibold transition-all"
                          style={isSel
                            ? { background: 'rgba(0,200,255,0.12)', color: '#00c8ff', borderColor: 'rgba(0,200,255,0.5)', boxShadow: '0 0 12px rgba(0,200,255,0.2)' }
                            : { background: '#08080f', color: '#7a7a9a', borderColor: '#1a1a2e' }}>
                          ...
                          <div className="text-left">
                            <p style={{ color: isSel ? '#00c8ff' : '#c0c0d0' }}>{key}</p>
                            {item.name && <p className="text-[10px] font-normal" style={{ color: isSel ? 'rgba(0,200,255,0.7)' : '#5a5a7a' }}>{item.name}</p>}
                          </div>
                        </button>
                      )
                    })}
                  </div>
                  {/* Warning if items not selected yet */}
                  {itemsRequired && selectedItems.length === 0 && selectedProfiles.length > 0 && (
                    <p className="text-vault-red text-xs font-mono mt-2">⚠ Please select at least one item to continue</p>
                  )}
                </div>
              )}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[11px] font-mono text-vault-muted uppercase tracking-widest">Select Profiles <span className="text-vault-red ml-1">*</span></p>
                  {selectedProfiles.length > 0 && <span className="text-[11px] font-mono text-vault-accent">{selectedProfiles.length} selected</span>}
                </div>
                {myProfiles.length === 0 ? (
                  <div className="p-4 rounded-xl border border-vault-border text-center"><p className="text-vault-text-dim text-sm">No profiles yet — add them on the Profiles page</p></div>
                ) : (
                    ...
                    {myProfiles.map(p => {
                      ...
                      return (
                        <button key={p.id} type="button" onClick={() => toggleProfile(p.id)}
                          ...
                          style={isSel ? { background: 'rgba(0,200,255,0.06)', borderColor: 'rgba(0,200,255,0.35)' } : { background: '#08080f', borderColor: '#1a1a2e' }}>
                          <div className="w-4 h-4 rounded border flex items-center justify-center shrink-0"
                            style={isSel ? { background: '#00c8ff', borderColor: '#00c8ff' } : { borderColor: '#3a3a5a' }}>
                            {isSel && <Check className="w-2.5 h-2.5" style={{ color: '#08080f' }} />}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs font-mono text-vault-muted truncate">{p.shipping_zip} · {p.card_type} ···· {p.card_number?.slice(-4) || '????'}</p>
                          </div>
              ...
              {/* Note */}
              <div>
                <label className="text-[11px] font-mono text-vault-muted uppercase tracking-widest block mb-2">Note to Admin <span className="normal-case opacity-60">(optional)</span></label>
                <input className="vault-input text-sm" placeholder="e.g. Profile 3 card may be virtual..." value={submissionNote} onChange={e => { setSubmissionNote(e.target.value); setSubmitted(false) }} />
              </div>
            </div>

            {/* Footer */}
            <div className="shrink-0 px-4 sm:px-6 py-3 sm:py-4 space-y-2" style={{ borderTop: '1px solid #1a1a2e', background: '#08080f' }}>
              ...
              <div className="flex gap-3">
                <button onClick={() => setSubmitModal(null)} className="px-4 py-2.5 rounded-lg text-sm text-vault-text-dim hover:text-vault-text hover:bg-vault-border transition-all">Cancel</button>
                <button onClick={handleSubmit} disabled={!canSubmit || submitting || submitted}
                  ...
                  style={{ background: submitted ? 'rgba(0,227,150,0.1)' : canSubmit ? '#00c8ff' : '#1a2a3a', color: submitted ? '#00e396' : canSubmit ? '#08080f' : '#4a6a8a', border: submitted ? '1px solid rgba(0,227,150,0.3)' : 'none' }}>
                  {submitting
                    ? <><div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />Submitting...</>
                    : submitted
                    ? <><CheckCircle className="w-4 h-4" />Submitted!</>
                    : itemsRequired && selectedItems.length === 0
